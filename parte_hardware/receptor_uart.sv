...
// Recebe 1 byte pela linha serial (rx): bit de start, 8 bits de dado, bit de stop.
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (estado_atual)
                ...
                OCIOSO: begin
                    // Espera a linha cair (bit de start) pra comecar a receber
                    contador_clock <= 16'd0;
                    indice_bit     <= 3'd0;
                    if (rx == 1'b0) begin
                        estado_atual <= INICIO;
                ...
                INICIO: begin
                    // Confirma o start no meio do bit (evita ruido/glitch curto)
                ...
                DADOS: begin
                    // Amostra 1 bit por vez, do menos significativo pro mais
                ...
                PARADA: begin
                    // Byte completo: avisa e volta a esperar o proximo
                    if (contador_clock < CLOCKS_POR_BIT - 1) begin
                        contador_clock <= contador_clock + 16'd1;
                        estado_atual   <= PARADA;
                    end else begin
