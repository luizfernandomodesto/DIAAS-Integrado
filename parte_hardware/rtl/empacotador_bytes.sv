...
// Junta 4 bytes recebidos (um de cada vez) numa palavra de 32 bits.
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            if (dado_valido_8b) begin
                // Desloca o byte novo pra dentro, os antigos vao pra esquerda
                dado_32b <= {dado_32b[23:0], dado_8b};
                contador_bytes <= contador_bytes + 2'd1;

                if (contador_bytes == 2'd3) begin
                    dado_valido_32b <= 1'b1; // 4o byte: a palavra esta completa
                end
            end
        end
    end
